        ...
        <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-5 grid gap-3">
            {client.signatureRequests.map((request) => {
              ...
              const publicSignaturePath = buildPublicSignaturePath(request.publicToken);
              const signedDocumentPath = request.signedDocument
                ? buildAdminSignedDocumentPath(request.id)
                : null;
              const timelineLabel = request.signedAt
                ? "Assinado"
                : request.expiresAt
                  ? "Expira"
                  : "Atualizado";
              const timelineValue = request.signedAt
                ? request.signedAt.toLocaleString("pt-BR", {
                    dateStyle: "short",
                    timeStyle: "short",
                  })
                : request.expiresAt
                  ? request.expiresAt.toLocaleDateString("pt-BR")
                  : request.updatedAt.toLocaleDateString("pt-BR");
                ...
                >
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0">
                      ...
                      <p className="mt-2 text-sm leading-6 text-slate-500">{serviceSummary}</p>
                      <p className="mt-2 text-xs font-medium uppercase tracking-[0.14em] text-slate-400">
                        {timelineLabel}
                      </p>
                      <p className="mt-1 text-sm text-slate-600">{timelineValue}</p>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 lg:max-w-[260px] lg:justify-end">
                      ...
                      {signedDocumentPath ? (
                        <Link
                          href={`${signedDocumentPath}?download=1`}
                          className="inline-flex items-center justify-center rounded-xl border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-700 transition hover:bg-white"
                        >
                          Baixar PDF
                        </Link>
